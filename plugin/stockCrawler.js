...
async function stockYield({stockno,yielddata}){
  ...
  }else if(yielddata && yielddata.slice(-1)[0]['date']<before_year){
    console.log(`stockYield,有值,前年${before_year}和資料年${yielddata.slice(-1)[0]['date']}不同抓取股利`)
    return await yield(before_year);
  }else{
...
async function stockCrawler_market({id,threecargo,threefutures,exdividend,listed,updownnumber,holder,retail,prosperity}){
